void Tree::buildTree(
    ...
    if( j == 0 ){
        ...
        if(subtree.get_s_prime() == 1){
            ...
            double prob = (subsubtree.get_n_prime() == 0.0 && subtree.get_n_prime() == 0.0) ? 0.0 : subsubtree.get_n_prime() / (subtree.get_n_prime() + subsubtree.get_n_prime());
            std::uniform_real_distribution<double> die(0.0,1.0);
            if(die(rng) <= prob)
...
void Tree::leapfrog(NHPP &model,
	...
	momenta_new = momenta + epsilon * beta_grad / 2.0;
