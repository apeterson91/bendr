...
```{r create_r}
## school_data available in the rndpp package
r <- school_data %>% arrange(school_id) %>%  ## sort to ensure correspondence with n_j
    select(distances) %>% pull()

n_j <- school_data %>% arrange(school_id) %>%
    group_by(school_id) %>% count() %>% 
