...
This vignette serves to introduce the `rndpp` R package, an R package that fits the Nested Dirichlet Process to realizations of an Inhomogenous Poisson Process. We begin with substantive motivation and theoretical notation to elucidate some key model components.  
...
Let's assume a simple constant $\gamma=10$ across all the schools, 50 from each intensity function.  
...
Estimation of the mean number of FFRs can be easily done via the `nhpp_hmc` function. As we said, this isn't anything particularly novel, so we simply provide the syntax below for a simple intercept model, but a more complicated regression could be fit.
...
This is not a fancy regression function, and can be emulated in other more standard libraries, e.g. `stats::glm`.
```{r, regression2}
