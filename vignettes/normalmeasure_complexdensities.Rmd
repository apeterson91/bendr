...
Let's assume a simple constant $\gamma$ across all the schools, simulating 50 from each intensity function.  
...
school_data %>% ggplot(aes(x=event_times)) + geom_density() + facet_wrap(~density) + 
...
```{r create_r}
r <- school_data %>% arrange(sim_id) %>%  ## sort to ensure correspondence with n_j
    select(event_times) %>% pull()

n_j <- school_data %>% arrange(sim_id) %>%
    group_by(sim_id) %>% count() %>% 
...
Estimation of the mean number of FFRs can be easily done via the `nhpp_hmc` function. As said, this isn't anything particularly novel, so we simply provide the syntax below for a simple intercept model, but a more complicated regression could be fit.
...
To provide a point of reference, this model can be emulated in standard libraries, e.g. `stats::glm`.
```{r, regression2}
